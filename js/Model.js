...
var VertexFormat = function(vertices, format) {
    ...
    // Create Attributes && Calculate Stride and Offset
...
var Mesh = function(gl, vertices, indices, format, primitive, calculateTangents) {
    ...
    if (true === calculateTangents) {
		this.vertices = this.calculateTangents(this, vertices, indices);
    }
...
Mesh.prototype.calculateTangents = function(mesh, vertices, indices) {
    ...
    if (this.primitive !== this.gl.TRIANGLES || 
	...
	var setQU = function(indices) 
	{
		...
		for (var j = 0; j < 3; j++) 
		{
			...
			for (var d = 0; d < 2; ++d) {
				tex[j * 2 + d] = vertices[vi * tStride + tOffset + d];
			}
		}
...
{
	// Resolves one to many mapping between vertex index and texture coordinates by adding new vertices
    ...
    $.each(data.objs, function() {
		...
		// Create a Mesh Object
      	var mesh = new Mesh(self.gl, vertices, indices, format, self.gl.TRIANGLES, true);
